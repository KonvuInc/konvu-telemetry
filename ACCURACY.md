...
Personal baselines use the median cumulative cost and token traffic of all sessions from the last 30 days. At least five sessions are required. A finished session retains its final cumulative total at later checkpoints, while still-active sessions continue accumulating. The dashboard records an exact median for every prompt through prompt 100, so short live sessions have a comparison; values above that are interpolated only between measured checkpoints and are suppressed outside the observed range.
