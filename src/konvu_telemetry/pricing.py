...
def load_pricing() -> dict[str, dict[str, float]]:
    ...
    try:
        raw: object = json.loads(pricing_path().read_text())
    ...
    for model, values in data.items():
        ...
        fast_multiplier = values.get("fastMultiplier", 1.0)
        price = {
            "input": float(input_rate),
            "output": float(output_rate),
            "cache_write": float(cache_write_rate)
            if isinstance(cache_write_rate, (int, float))
            else float(input_rate) * 1.25,
            "cache_read": float(cache_read_rate)
            if isinstance(cache_read_rate, (int, float))
            else float(input_rate) * 0.1,
            "web_search": float(web_search_rate)
            if isinstance(web_search_rate, (int, float))
            else 0.01,
            "fast_multiplier": float(fast_multiplier)
            if isinstance(fast_multiplier, (int, float))
            else 1.0,
            "context_window_tokens": float(values.get("max_input_tokens", 0))
            if isinstance(values.get("max_input_tokens"), (int, float))
            else 0.0,
            ...
            for suffix, label in (
                ...
                for variant in key_variants:
                    ...
                    if not isinstance(value, (int, float)):
                        continue
                    price[f"{field}_{label}"] = float(value)
